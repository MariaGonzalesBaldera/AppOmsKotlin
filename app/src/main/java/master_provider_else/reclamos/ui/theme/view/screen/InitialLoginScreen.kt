...
class InitialLoginScreen() : ComponentActivity() {
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    fun LoginScreen(navController: NavController){
      val userViewModel: UserViewModel by viewModels()
      Greeting1(userViewModel = userViewModel, navController=navController)
    }
    * */
...
@Composable
fun Greeting1(modifier: Modifier = Modifier,userViewModel: UserViewModel,navController: NavController) {
  val context = LocalContext.current
  val focusManager = LocalFocusManager.current
  var passwordVisibility by remember { mutableStateOf(false) }
  var checkedStateValue by remember { mutableStateOf("") }
          ...
          Box(
            ...
            BotonDefault(title = "Ingresar") {
              if(userValue.value.isNotBlank() || passwordValue.value.isNotBlank()){
                ...
                if(userViewModel.validModel.value == true){
